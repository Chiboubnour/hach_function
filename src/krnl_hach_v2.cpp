...
void krnl_hach(const ap_uint<INPUT_WIDTH>* sequence,int n,
               ap_uint<OUTPUT_WIDTH>* tab_hash){
    #pragma HLS INTERFACE m_axi port=sequence offset=slave bundle=gmem_seq max_read_burst_length=256 num_read_outstanding=32
    #pragma HLS INTERFACE m_axi port=tab_hash offset=slave bundle=gmem_out max_write_burst_length=256 num_write_outstanding=32
