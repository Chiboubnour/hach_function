...
// Packager la séquence dans des uint64_t
size_t input_size_bytes = ((n + 7)/8) * sizeof(uint64_t);
std::vector<uint64_t> packed_seq(input_size_bytes / sizeof(uint64_t), 0);

for (size_t i = 0; i < n; i++) {
size_t word_idx = i / 8;
size_t shift = 8 * (i % 8);
packed_seq[word_idx] |= (uint64_t)sequence_bytes[i] << shift;
}
